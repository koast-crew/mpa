import { SquareUserRound, FishSymbol, Map } from 'lucide-react';
  ...
  return (
    <div className={'box-border flex size-full flex-col border-t border-zinc-500 bg-slate-800'}>
      <Link to={'/'} className={'box-border flex h-[65px] w-full flex-col items-center justify-center border-b border-zinc-500' + (pathname === '/' ? ' bg-blue text-light' : '')}>
        <SquareUserRound className={'text-light'} width={'20'} height={'20'} />
        <div className={'m-[2px] text-[12px] font-bold text-light'}>{'관리자'}</div>
      </Link>
      ...
      <Link to={'/nav3'} className={'box-border flex h-[65px] w-full flex-col items-center justify-center border-b border-zinc-500' + (pathname === '/nav3' ? ' bg-blue text-light' : '')}>
